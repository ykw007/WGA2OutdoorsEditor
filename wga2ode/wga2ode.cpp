// WGA2ode.cpp : 해당 DLL의 초기화 루틴을 정의합니다.
//
...
static AFX_EXTENSION_MODULE WGA2odeDLL = { NULL, NULL };
...
DllMain(HINSTANCE hInstance, DWORD dwReason, LPVOID lpReserved)
{
	...
	{
		TRACE0("WGA2ode.DLL 초기화!\n");
		
		// 확장 DLL을 한 번만 초기화합니다.
		if (!AfxInitExtensionModule(WGA2odeDLL, hInstance))
			return 0;
		...
		new CDynLinkLibrary(WGA2odeDLL);

	}
	else if (dwReason == DLL_PROCESS_DETACH)
	{
		TRACE0("WGA2ode.DLL 종료!\n");

		// 소멸자가 호출되기 전에 라이브러리를 종료합니다.
		AfxTermExtensionModule(WGA2odeDLL);
	}
	return 1;   // 확인
}
